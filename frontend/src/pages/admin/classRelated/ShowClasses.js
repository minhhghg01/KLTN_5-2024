...
const ShowClasses = () => {
  ...
  const SclassButtonHaver = ({ row }) => {
    const actions = [
      ...
      { icon: <PersonAddAlt1Icon />, name: 'Thêm học viên', action: () => navigate("/Admin/class/addstudents/" + row.id) },
    ];
    return (
      <ButtonContainer>
    ...
    return (
      <>
        <Box sx={{ display: 'flex', alignItems: 'center', textAlign: 'center' }}>
          <Tooltip title="Thêm học sinh & môn học">
            <IconButton
              onClick={handleClick}
              size="small"
              sx={{ ml: 2 }}
  ...
  const actions = [
    {
      icon: <AddCardIcon color="primary" />, name: 'Thêm lớp',
      action: () => navigate("/Admin/addclass")
    },
    {
      icon: <DeleteIcon color="error" />, name: 'Xóa lớp',
